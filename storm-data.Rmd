...
# Ensure we got the data downloaded, decompressed and loaded correctly by checking filesize and dataset dimemsions
...
3. With the mulitplier variables created, `CropDamage` and `PropDamage` variables are added by multiplying them against the corresponding damage variables `PROPDMG` and `CROPDMG`.  In addition, a `TotalDamage` variable is  also added, using the sum of both the crop and property damage.
...
4. For determining the oveall health impact of events, a `PopulationHealthImpact` variable is added using the
...
5. To make the dataset easier to work with, irrelevant observations are removed.  According to the documentation, it was not until 1996 that all event types were being recorded.  For that reason, the years from the dataset earlier than that are removed in order to get a fair assessment of all events. Also, since we are answering questions around economic and population health impact, all rows having neither of these are removed as well.  
...
6. Looking at the top events with the most `TotalDamage` and `PopulationHealthImpact` showed that there was a least one
...
kable(
  head(
    sd[order(-sd$TotalDamage), 
              c('REFNUM', 'STATE', 'BeginDate', 'EVTYPE', 'CROPDMG', 
                'CROPDMGEXP', 'PROPDMG', 'PROPDMGEXP', 'TotalDamage')],
    n =5
    ), 
  row.names = FALSE,
  caption = 'Top Five Events by Total Economic Damage (after fixing REFNUM 605943)'
)
```
...
7. Given the poor consistency of the values found in the `EVTYPE` variable, it was decided to use the list of Event names from Section 2.1.1 
...
health_by_eventtype_and_year <- aggregate(
  cbind(FATALITIES, INJURIES, PopulationHealthImpact) ~ EventType + year(BeginDate), 
  sd, 
  FUN=sum
)
names(health_by_eventtype_and_year) <- c('EventType', 'Year', 'Fatalities', 'Injuries', 'PopulationHealthImpact')
health_by_eventtype_and_year <- health_by_eventtype_and_year[health_by_eventtype_and_year$EventType %in% top_health$EventType,]
health_by_eventtype_and_year$EventType <- with(health_by_eventtype_and_year, reorder(EventType, -PopulationHealthImpact))

ggplot(health_by_eventtype_and_year, aes(x=Year, y=PopulationHealthImpact, colour = EventType)) + 
  geom_point() + geom_line() +
  scale_x_continuous(breaks = unique(health_by_eventtype_and_year$Year)) +
  scale_y_continuous(
...
damage_by_eventtype_and_year <-  aggregate(
  cbind(CropDamage,PropDamage,TotalDamage)~EventType+year(BeginDate), 
  sd, 
  FUN=sum
)
names(damage_by_eventtype_and_year) <- c('EventType', 'Year', 'CropDamage', 'PropDamage', 'TotalDamage')

sd_dmg_yearly <- damage_by_eventtype_and_year[damage_by_eventtype_and_year$EventType %in% top_damage$EventType,]
...
```{r results = 'asis'}
ksd <- unique(subset(sd[sd$EventType != 'UNCATEGORIZED',], select=c(EventType, EVTYPE)))
ksd <- ksd[order(ksd$EventType, ksd$EVTYPE),]

for (eventType in unique(ksd$EventType)) {
  if(length(ksd$EVTYPE[ksd$EventType==eventType]) > 1) {
    k <- kable(
      ksd$EVTYPE[ksd$EventType==eventType], 
      row.names = FALSE, 
      col.names = eventType
    )
    print(k)
